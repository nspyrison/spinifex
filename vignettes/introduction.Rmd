...
* 'phi', angle controlling the magnitude of change of `manip var`, passed as a sequence of the form `seq(from, to, by)` 
...
Use `data_proj()` to rotate `data` at the current orientation `basis`, by manipulating `manip_var` of the `manip` type, for a sequence of 'phi' `from` thru `to` by an increment `by`.
...
g2 <-
  ggplot2::ggplot(temp.gly2, ggplot2::aes(gx, gy, group = gid)) +
  GGally::add_ref_lines(temp.gly, color = "grey90") +
  GGally::add_ref_boxes(temp.gly, color = "grey90") +
  ggplot2::geom_path() +
  ggplot2::theme_bw() +
