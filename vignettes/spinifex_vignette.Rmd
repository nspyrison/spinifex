...
```{r}
# Initialize
nasa_std <- cbind(nasa[c("x", "y")], tourr::rescale(nasa[c("day", "surftemp")]))
...
# rot_xy <- select(rot_xy, x, y1)
# rot_xy <- rename(rot_xy, x=x, y=y1)
# 
...
tour_x <- manual_tour(basis = bas, manip_var = 3, manip_type = "horizontal")
slides_x <- create_slides(tour = tour_x, data = nasa_std)
# render_plotly(slides = slides_x) # Way too heavy

# Vertical manual rotation
tour_y <- manual_tour(basis = bas, manip_var = 4, manip_type = "vertical")
slides_y <- create_slides(tour = tour_y, data = nasa_std)
...
```{r}
# render_plotly(tr1_sshow)
# render_plotly(tr2_sshow)
# render_plotly(hed_sshow)
# render_plotly(ad1_sshow)
# render_plotly(ad2_sshow)
# render_plotly(ad3_sshow)
```
