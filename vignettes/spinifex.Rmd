...
ggtour(basis_array = bas_pca, data = dat_std) + proto_default()
```
...
play_tour_path(tour_path = fpath, data = dat_std, angle = .15,
               aes_args = list(color = clas, shape = clas),
               render_type = render_gganimate, fps = 8)
```
...
f_holes_bas <- basis_guided(dat_std, index_f = tourr::holes(), d = 2)

view_frame(f_holes_bas)
```
...
(mv <- manip_var_of(f_holes_bas, rank = 1))
