#' @param start_basis A full (p x 2) basis to start removing variables from.
...
stepwise_hist <- function(start_basis = NULL,
  ...
  #for(i in (p - 1L):2L){
    ...
    is_orthonormal(new_bas)
    
    basis_ls[[i]] <<- new_bas ## Add next lvl assignment when vectorizing
